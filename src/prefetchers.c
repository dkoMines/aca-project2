                                  ...
                                  uint32_t address, bool is_miss)
{
    ...
    if ((float)adapt[3]/adapt[9] > percentNeeded){
        if (cache_system_mem_access(cache_system, address + oneBlock, 'R', true) == 0){
            count ++;
        }
    }
    if ((float)adapt[4]/adapt[9] > percentNeeded){
        if (cache_system_mem_access(cache_system, address - oneBlock, 'R', true) == 0){
            count ++;
        }
    }
    if ((float)adapt[5]/adapt[9] > percentNeeded){
        if (cache_system_mem_access(cache_system, address + oneSet*2, 'R', true) == 0){
            count ++;
        }
    }
    if ((float)adapt[6]/adapt[9] > percentNeeded){
        if (cache_system_mem_access(cache_system, address - oneSet*2, 'R', true) == 0){
            count ++;
        }
    }
    if ((float)adapt[7]/adapt[9] > percentNeeded){
        if (cache_system_mem_access(cache_system, address + oneBlock*2, 'R', true) == 0){
            count ++;
        }
    }
    if ((float)adapt[8]/adapt[9] > percentNeeded){
        if (cache_system_mem_access(cache_system, address - oneBlock*2, 'R', true) == 0){
            count ++;
        }
    }
